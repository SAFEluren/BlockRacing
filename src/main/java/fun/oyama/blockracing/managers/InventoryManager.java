...
public class InventoryManager {
    ...
    public static final String ROLL = ChatColor.YELLOW + "刷新";
    ...
    public static void setLocateItem() {
        ...
        builder.setLore(ChatColor.AQUA + "花费" + GameManager.locateCost + "积分获得一次locate命令使用权限", ChatColor.RED + "注意！如果距离过远可能会引发服务器卡顿！");
        builder.toItemStack();
    ...
    public static void setItem(String material, int amount, String displayName, String lore, int slot, String inv) {
        ...
        builder.toItemStack();
        switch (inv) {
            case "settings" -> settings.setItem(slot, stack);
            case "menu" -> menu.setItem(slot, stack);
            case "chestSwitch" -> chestSwitch.setItem(slot, stack);
            case "redWayPoints" -> redWayPoints.setItem(slot, stack);
            case "blueWayPoints" -> blueWayPoints.setItem(slot, stack);
        }
    }
}
